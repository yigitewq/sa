...
app.get("/sitemap.xml", async function(req,res) {
  ...
  const renderTemplate = (res, req, template, data = {}) => {
    ...
    app.get("/callback", passport.authenticate("discord", { failureRedirect: "/error?code=999&message=We encountered an error while connecting." }), async (req, res) => {
        ...
        client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({dynamic: true})).setThumbnail(a.avatarURL({dynamic: true})).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](https://https://bot-list-1.shazyigit.repl.co/user/${a.id}) isimli kullanıcı **siteye** giriş yapmaya çalıştı fakat siteden engellendiği için giriş yapamadı.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
        })
        req.session.destroy(() => {
        res.json({ login: false, message: "ShazY Botlist ten Bloklandın!", logout: true })
        req.logout();
        });
        } else {
        res.redirect(req.session.backURL || '/')
        client.users.fetch(req.user.id).then(async a => {
        client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({dynamic: true})).setThumbnail(a.avatarURL({dynamic: true})).setColor("GREEN").setDescription(`[**${a.username}**#${a.discriminator}](https://bot-list-1.shazyigit.repl.co/${a.id}) isimli kullanıcı **siteye** giriş yaptı.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
    ...
    app.get("/dc", (req, res) => {
      res.redirect('https://discord.gg/mZp9RFEds6');
    });  
    ...
    app.get("/partners", checkMaintence, (req, res) => {
      const Database = require("shazy.db");
      const db = new Database("./partnerdatabase.json")
    ...
    app.post("/code-request", checkMaintence, checkAuth, async (req,res) => {
       client.users.fetch(req.user.id).then(a => {
         ...
         client.channels.cache.get(channels.request).send(new Discord.MessageEmbed()
                              ...
                              .setDescription(`User **[${a.username}#${a.discriminator}](https://bot-list-1.shazyigit.repl.co/${a.id})** requested the code named **${rBody['kodadi']}**.`)
    ...
    app.post("/uptime/add", checkMaintence, checkAuth, async (req,res) => {
      ...
      if(!rBody['link']) { 
          ...
          .setDescription("Yeni Bir Link Eklendi Sisteme.")
        ...
        res.redirect('?success=true&message=Senin Linkin Başarıyla Sisteme Eklendi.');
      }
    })
    ...
    app.get("/bot/:botID/delete", async (req, res) => {
      ...
      app.post("/admin/decline/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
       ...
       app.post("/admin/addcode", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
          ...
          .setDescription(`The user named **[${req.user.username}](https://bot-list-1.shazyigit.repl.co/${req.user.id})** added the code named **${rBody['codename']}** to the system.`)
       ...
       app.post("/admin/editcode/:code", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
          ...
          .setDescription(`The user named **[${req.user.username}](https://bot-list-1.shazyigit.repl.co/${req.user.id})** edited the code named **${rBody['codename']}**.`)
          .addField("Code Link", `https://vcodes.xyz/code/${kod}`, true)
          .addField("Code Description", rBody['codedesc'], true)
          .addField("Code Category", rBody['category'], true)
          )
